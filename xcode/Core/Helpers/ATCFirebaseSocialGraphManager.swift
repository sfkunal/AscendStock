        ...
        reportingManager.userIDsBlockedOrReported(by: viewer) { (illegalUserIDsSet) in
            ...
            ref.getDocuments { (querySnapshot, error) in
                ...
                ref2.getDocuments { (querySnapshot, error) in
                    ...
                    for userID in allUniqueUsersIDsToBeHydrated {
                        usersRef.document(userID).getDocument(completion: { (document, error) in
                            if let userDict = document?.data() {
                                ...
                                if hydratedUsers.count == allUniqueUsersIDsToBeHydrated.count {
                                    ...
                                    let friendships = friendsIDs.map({ATCChatFriendship(currentUser: viewer, otherUser: self.user(for: $0, in: hydratedUsers), type: .mutual)})
                                    // Then we add the inbound requests
                                    let inboundRequests = inboundRequestsUserIDs.map({ATCChatFriendship(currentUser: viewer, otherUser: self.user(for: $0, in: hydratedUsers), type: .inbound)})
                                    // Then we add the outbound requests
                                    let outboundRequests = outboundRequestsUserIDs.map({ATCChatFriendship(currentUser: viewer, otherUser: self.user(for: $0, in: hydratedUsers), type: .outbound)})
